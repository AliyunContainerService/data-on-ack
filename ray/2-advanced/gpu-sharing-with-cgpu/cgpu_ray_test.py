...
It checks three things:
  ...
  2. A GPU actor sees a device whose *total* memory equals the cGPU slice
     (e.g. ~8 GiB), not the whole physical card, confirming cGPU isolation.
  3. Allocating within the slice succeeds; allocating beyond it is rejected
     with an out-of-memory error, confirming the cap is enforced.
"""
