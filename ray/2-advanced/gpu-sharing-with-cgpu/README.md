...
Ray does not know about `aliyun.com/gpu-mem`. This guide shows the one
non-obvious change needed to make a KubeRay worker land on a cGPU slice and
expose it to Ray as a normal GPU, plus how to verify the isolation actually
works.

Validated on ACK Pro (K8s 1.34) with one `ecs.gn7i-c8g1.2xlarge` node (NVIDIA
A10, 22 GiB), sharing an 8 GiB slice into a Ray worker.
...
KubeRay computes 0 GPUs, Ray starts with `num-gpus=0`, and any
`@ray.remote(num_gpus=1)` actor stays forever Pending.

The fix is to declare the GPU count yourself in that worker group's
`rayStartParams`:
...
kernel module caps the pod's visible GPU memory to the requested value — so
inside Ray it behaves like a small dedicated GPU.
...
it got and then allocates within and beyond the slice.
...
a 4 GiB tensor fits, and a 16 GiB tensor is rejected with an out-of-memory
error — the cGPU cap is enforced, from within Ray.

## Notes

- **Packing multiple actors onto one slice**: request the slice once at the
  pod level (`aliyun.com/gpu-mem`), keep `num-gpus: "1"`, and give each actor a
  fractional share with `@ray.remote(num_gpus=0.5)`. Ray's fraction is logical
  bookkeeping within the pod; the hard memory cap is the cGPU slice shared by
  all actors in that pod.
- **Sharing one physical GPU across pods**: schedule several cGPU worker pods
  (each requesting a slice) onto the same node; the gpushare scheduler packs
  them by `aliyun.com/gpu-mem` and cGPU isolates each. They can belong to
...
- **Compute isolation**: labelling the node `ack.node.gpu.schedule=core_mem`
  ...
  compute-share limits; the Ray wiring is identical.
- **Whole-GPU workers** in the same cluster still use `nvidia.com/gpu` and need
  no `num-gpus` override — KubeRay infers those normally.
