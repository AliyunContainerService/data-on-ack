...
not work out of the box on eRDMA. This directory records what it takes to make
NIXL work, and what you get for the effort. Both paths below were validated on
two `ecs.ebmgn9gc.64xlarge` nodes (RTX PRO 5000, two eRDMA interfaces,
~380 Gbps in total).

## The short path: use the prebuilt image

Everything below is baked into an image by [`Dockerfile`](Dockerfile), built
...
so `ucx_perftest` and `import nixl` work out of the box.
...
Two gotchas found the hard way: the UCX build must be 1.21 (1.19 is missing
...
## Is it really RDMA, or has UCX fallen back to TCP?

Worth checking before trusting any of these numbers, since UCX falls back
silently. [`verify_transport.sh`](verify_transport.sh) settles it two ways at
once: it diffs the eRDMA device byte counters around each run, and records the
...
The counters move by the payload volume on the RDMA rows and by exactly nothing
on the TCP rows, so the RDMA numbers are real. Two uncomfortable consequences:

- **RDMA only wins for the DRAM WRITE case** (19.3 vs 5.7 GiB/s). For a
  one-sided READ of host memory, plain TCP is *faster* than eRDMA
  (3.6 vs 2.7 GiB/s).
- **For GPU memory, TCP beats eRDMA in both directions** (0.54-0.58 vs
  0.30-0.41 GiB/s). The `cuda_copy` staging path is so dominant that the fabric
...
descriptor count; it produced the numbers above. Three findings are worth
having before concluding anything about NIXL's speed.

**RDMA READ is the slow part, not NIXL.** A one-sided READ of 256 MiB gets
2.7 GiB/s while the very same stack doing a WRITE gets 20.4 GiB/s. Raw UCX
agrees, and is in fact *slower* than NIXL on the same primitive:
...
[`nixl_check.py`](nixl_check.py) does - reports about an eighth of what the
link can do. Use WRITE for bulk data on eRDMA.

**No UCX knob rescues the GPU path.** [`ucx_knob_sweep.sh`](ucx_knob_sweep.sh)
...
beats the default by more than noise, while several halve throughput
...
(`UCX_NET_DEVICES=erdma_0:1,erdma_1:1` with `UCX_MAX_RNDV_RAILS=2`) is not an
option either: the agent starts but the first transfer aborts on a UCX
...
descriptors changes nothing either. Both are the obvious suspects and both are
dead ends here.

**GPU memory is the real cliff.** Without GPUDirect RDMA, UCX stages every GPU
...
DRAM WRITE path on the same link. That, not the fabric, is why NIXL loses to
NCCL for GPU tensors: NCCL does its own staging with a dedicated proxy thread,
pre-registered bounce buffers and many parallel channels, and reaches
11.5 GiB/s GPU-to-GPU between the same two nodes.
...
The NIXL/Mooncake crash is a **Ray-side limitation**: Ray 2.57's NIXL
integration initializes all backends found in `NIXL_PLUGIN_DIR` within the same
process; with both UCX and Mooncake present, two independent RDMA stacks fight
over QP resources and the Mooncake-side UCX endpoint hits `error modifying QP to
RTR`. Outside Ray (the `nixl_check.py` / `nixl_tuning_sweep.py` scripts in this
directory) the Mooncake backend alone reaches 19 GiB/s GPU WRITE and 12 GiB/s
GPU READ — it just cannot coexist with the UCX backend in the same process yet.
...
time (see "Where the throughput goes" above), while the hardware can sustain
4× that (`ib_read_bw -q 4` = 215 Gb/s). A future UCX that pipelines multiple
outstanding READs would close this gap.

## Limitations

- **NCCL with GDR is still the fastest end-to-end choice for GPU tensors in
  Ray** (11 GiB/s versus NIXL/UCX's 6.2 GiB/s). Use NIXL only when something
  else in your stack requires it (e.g. vLLM PD disaggregation).
- **NIXL/Mooncake does not work inside Ray** due to the multi-backend
  initialization conflict described above. Outside Ray it delivers 19 GiB/s GPU
  WRITE, the fastest measured path — so if Mooncake integration is needed,
  build NIXL with only the Mooncake plugin (drop `libplugin_UCX.so` from the
  plugin directory).
- **GDR must be present** for any of the GPU numbers above. Without it (e.g.
  when the erdma module was rebuilt against in-tree ib_core, losing
  `ib_umem_get_peer`), VRAM paths collapse to < 1 GiB/s. The nodes must run the
  original OFED RDMA stack + `nvidia_peermem`.
- Everything here needs a patched UCX or a source-built Mooncake, so it only
  makes sense if you are baking a custom image, which is what
  [`Dockerfile`](Dockerfile) and [`build-job.yaml`](build-job.yaml) do.
