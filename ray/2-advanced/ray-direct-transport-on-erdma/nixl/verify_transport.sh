#!/bin/bash
# Prove which transport a NIXL run actually used, rather than trusting UCX_TLS.
#
# For each configuration: read the eRDMA byte counters of both devices before
# and after the run, and record the lane UCX selected. A run that really used
# eRDMA moves the counters by the payload volume; a TCP run leaves them flat.
#
