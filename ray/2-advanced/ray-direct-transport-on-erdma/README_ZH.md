...
> **基础镜像说明：** 由于国内地域无法直连 docker.io，本文使用 `rayproject/ray-llm:2.57.0-py312-cu130` 的转投副本 `rayproject/ray-llm:2.57.0-py312-cu130`。该转投仓库只有拉取权限，所以派生镜像推到了自己的 ACR 仓库。请把镜像仓库地址和 Secret 名称替换为你自己的。
...
要区分「eRDMA/NCCL 本身的问题」与「Ray RDT 配置的问题」,可以在不经过 Ray 的情况下运行一次 NCCL:[`erdma-nccl-check-pods.yaml`](erdma-nccl-check-pods.yaml) 在两个 eRDMA 节点上各部署一个独立 Pod,[`nccl_allreduce_check.py`](nccl_allreduce_check.py) 在两者之间运行两个 rank 的 `all_reduce`(用法见脚本 docstring)。实测环境下 0.5 GiB 的 `all_reduce` 经 eRDMA 约 80–90 ms;设置 `NCCL_IB_DISABLE=1` 后耗时接近,但设备计数器保持不变——据此可区分两条路径。

## 其他传输后端

- **NIXL** 无法直接使用官方预编译 wheel:wheel 自带 UCX 和改名后的 `libibverbs`,导致 UCX 无法识别 RDMA 设备;即使解决该问题,UCX 的 `rc_verbs` 传输仍会强制创建共享接收队列,而 eRDMA 不支持 SRQ(`ibv_create_srq() failed: Operation not supported`)。NIXL 可通过两种方式在 eRDMA 上工作:一是使用阿里云的 eRDMA patch 重新编译 UCX,二是源码构建带 Mooncake 后端的 NIXL。两种方案的做法与实测数据见 [`nixl/`](nixl/)。性能上 NIXL/UCX 的 GPU 间传输约 0.84 GiB/s,而 NCCL 约 11.5 GiB/s,因此仅在技术栈确有需要时才建议使用 NIXL。
- **GLOO** 可用,但属于 CPU 传输:张量需先拷贝到 host 内存,不适用于本场景。
...
- **GPUDirect RDMA 在这类实例上未启用**(`NET/IB : GPU Direct RDMA Disabled for HCA 0 'erdma_0'`),数据仍需经 host 内存中转。此处开销较小,因为 PCIe 带宽远高于网卡,且 NCCL 会将中转拷贝与网络传输重叠;若中转实现未经优化,吞吐会降至 1 GiB/s 以下——NIXL/UCX 即属此情况(见 [`nixl/`](nixl/))。
- **hostNetwork** 意味着每个节点最多运行一个 Ray worker Pod,`replicas` 应按节点数设置。
- **每个 Actor 对每种后端只能加入一个 collective group**。传输失败时 Ray 会销毁该 group,失败的运行可能残留未清理的 `NCCLUniqueIDStore` Actor,重试前需先清理。
