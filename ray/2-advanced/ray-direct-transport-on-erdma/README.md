...
3. Run an RDT demo that moves a 512 MiB GPU tensor between two nodes over eRDMA, and verify that the traffic used the RDMA path.
...
> **Which transport?** On eRDMA, use `tensor_transport="nccl"`. The NIXL transport does not work out of the box: NIXL's UCX backend needs a UD queue pair to bootstrap its RC connections, and eRDMA does not implement UD (`uct_iface_open(ud_verbs/erdma_0:1) failed: Address not valid`). See [Other Transports](#other-transports).
...
Two requirements that are easy to overlook:
...
- **GPUDirect RDMA is disabled** on these instances (`NET/IB : GPU Direct RDMA Disabled for HCA 0 'erdma_0'`), so data still passes through host memory. The overhead is small here because PCIe bandwidth far exceeds the NIC and NCCL overlaps the staging copies with the transfers; if the staging is not optimized, throughput drops below 1 GiB/s, which is the case for NIXL/UCX (see [`nixl/`](nixl/)).
