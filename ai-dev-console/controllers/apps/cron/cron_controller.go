...
import (
	...
	marsv1alpha1 "github.com/AliyunContainerService/data-on-ack/ai-dev-console/apis/mars/v1alpha1"
	//mpiv1 "github.com/kubeflow/mpi-operator/apis/kubeflow/v1alpha1"
	//xdlv1alpha1 "github.com/AliyunContainerService/data-on-ack/ai-dev-console/apis/xdl/v1alpha1"
	//xgboostv1alpha1 "github.com/AliyunContainerService/data-on-ack/ai-dev-console/apis/xgboost/v1alpha1"
	...
	"github.com/AliyunContainerService/data-on-ack/ai-dev-console/pkg/job_controller"
	...
	utilruntime "github.com/AliyunContainerService/data-on-ack/ai-dev-console/pkg/util/runtime"
...
func NewCronController(mgr ctrl.Manager, config job_controller.JobControllerConfiguration) *CronController {
...
func (cc *CronController) refreshCronHistory(cron *v1alpha1.Cron, workloads []metav1.Object) error {
	...
	if cron.Spec.HistoryLimit != nil && historySize > int(*cron.Spec.HistoryLimit) {
		klog.Infof("history size of cron %s/%s exceeds history limit %d, truncate it",
			cron.Namespace, cron.Name, *cron.Spec.HistoryLimit)

		//toTruncate := cron.Status.History[*cron.Spec.HistoryLimit:]
		toTruncate := cron.Status.History[:historySize-int(*cron.Spec.HistoryLimit)]
		for _, truncate := range toTruncate {
			if err := cc.deleteWorkload(cron, corev1.ObjectReference{
				UID:        truncate.UID,
...
func (cc *CronController) deleteWorkload(cron *v1alpha1.Cron, ref corev1.ObjectReference) error {
	...
	deleteFromActiveList(cron, ref.UID)
	err = cc.client.Delete(context.Background(), wl)
	if err != nil {
		klog.Errorf("delete workload %s/%s failed, err: %v", ref.Namespace, ref.Name, err)
	}
