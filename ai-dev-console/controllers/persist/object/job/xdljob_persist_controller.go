...
func (pc *XDLJobPersistController) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			if err = pc.handler.Delete(req.Namespace, name, xdlJob.Kind, id); err != nil {
				return ctrl.Result{}, err
			}
	...
	if err = pc.handler.Write(&xdlJob, xdlJob.Kind, xdlJob.Spec.XDLReplicaSpecs, &xdlJob.Spec.RunPolicy, &xdlJob.Status); err != nil {
...
func (pc *XDLJobPersistController) SetupWithManager(mgr ctrl.Manager) error {
	c, err := controller.New("XDLJobPersistController", mgr, controller.Options{
		Reconciler:              pc,
