...
import (
	...
	"github.com/AliyunContainerService/data-on-ack/ai-dev-console/apis/training/v1alpha1"
...
func (pc *XGBoostJobPersistController) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			if err = pc.handler.Delete(req.Namespace, name, xgboostJob.Kind, id); err != nil {
	...
	if err = pc.handler.Write(&xgboostJob, xgboostJob.Kind, xgboostJob.Spec.XGBReplicaSpecs, &xgboostJob.Spec.RunPolicy, &xgboostJob.Status.JobStatus); err != nil {
		return ctrl.Result{Requeue: true}, err
	}
	return ctrl.Result{}, nil
}
...
func (pc *XGBoostJobPersistController) SetupWithManager(mgr ctrl.Manager) error {
	c, err := controller.New("XGBoostJobPersistController", mgr, controller.Options{
		Reconciler:              pc,
