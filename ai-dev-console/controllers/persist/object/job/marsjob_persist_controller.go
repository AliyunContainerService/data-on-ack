...
func (pc *MarsJobPersistController) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			if err = pc.handler.Delete(req.Namespace, name, marsJob.Kind, id); err != nil {
				return ctrl.Result{}, err
			}
	...
	if err = pc.handler.Write(&marsJob, marsJob.Kind, marsJob.Spec.MarsReplicaSpecs, &marsJob.Spec.RunPolicy, &marsJob.Status.JobStatus); err != nil {
...
func (pc *MarsJobPersistController) SetupWithManager(mgr ctrl.Manager) error {
	c, err := controller.New("MarsJobPersistController", mgr, controller.Options{
		Reconciler:              pc,
