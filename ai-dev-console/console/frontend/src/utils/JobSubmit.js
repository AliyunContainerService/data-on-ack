...
export const handleImagePullSecrets = (info) => {
  const imgPullSecretsInfo = info?.kind === 'TFJob' ? Object.values(info?.spec?.tfReplicaSpecs)[0]?.template?.spec?.imagePullSecrets
      : Object.values(info?.spec?.pytorchReplicaSpecs)[0]?.template?.spec?.imagePullSecrets;
  return imgPullSecretsInfo;
}
