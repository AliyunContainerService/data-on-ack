...
class LogModal extends React.Component {
  ...
  async loadLogSilently() {
    ...
    if (this.state.currentKey === "events") {
      ...
      res = await getPodLog(job.namespace, job.jobType, job.name, pod.name, pod.podId, createTime);
    }
  ...
  handleDownload = e => {
    ...
    let createTime = moment(pod.createTime).toDate().toISOString();
    window.open(
      downloadLogURL(job.namespace, job.jobType, job.name, pod.name, pod.podId, createTime),
      "_blank"
    );
  };
