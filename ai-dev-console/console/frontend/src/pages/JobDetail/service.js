...
export async function getPodLog(namespace, jobType, jobName, podName, podId, podCreateTime) {
  return request(`${APIV1Prefix}/log/logs/${namespace}/${jobType}/${jobName}/${podName}`, {
    params: {
...
export function downloadLogURL(namespace, jobType, jobName, podName, podId, podCreateTime) {
  const fromTime = podCreateTime;
  return encodeURI(
    `${APIV1Prefix
    }/log/download/${namespace}/${jobType}/${jobName}/${podName}?fromTime=${fromTime}&uid=${podId}`,
  );
}
