...
var _ = Describe("PodControl", func() {
	It("Running status counting", func() {
		now := v1.Now()
		toCreate := training.TFJob{
			ObjectMeta: v1.ObjectMeta{
				Name:      "foo",
				Namespace: "default",
			},
			Spec: training.TFJobSpec{TFReplicaSpecs: map[apiv1.ReplicaType]*apiv1.ReplicaSpec{}},
		...
		pending, err := JobStatusCounter(training.TFJobKind, k8sClient, func(status apiv1.JobStatus) bool {
	...
	It("Pending status counting", func() {
		...
		running, err := JobStatusCounter(training.TFJobKind, k8sClient, util.IsRunning)
		Expect(err).Should(Succeed())
