...
func setDefaultWorkerMemTuningPolicy(job *MarsJob) {
	...
	if job.Spec.WorkerMemoryTuningPolicy.PlasmaStore == nil {
		job.Spec.WorkerMemoryTuningPolicy.PlasmaStore = pointer.StringPtr(DefaultCacheMountPath)
	}
	...
	if job.Spec.WorkerMemoryTuningPolicy.WorkerCacheSize == nil &&
		job.Spec.WorkerMemoryTuningPolicy.WorkerCachePercentage == nil {
		p := DefaultCacheSizePercentage
		job.Spec.WorkerMemoryTuningPolicy.WorkerCachePercentage = &p
	}
}
