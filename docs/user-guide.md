...
### AI 助手（Agentic Console，可选）

开发控制台内置基于 agentscope-go 的 AI 助手（需管理员启用并配置模型密钥）：

- **故障诊断**："我的任务为什么失败了" —— 助手自动拉取任务状态/事件/日志/指标，给出带引用证据的根因分析与修复建议
- **资源问答**："配额还够跑 8×A100 吗 / 为什么 Pending" —— 基于 ElasticQuotaTree 与实际用量
- **平台导航**：查询 Notebook / 训练任务 / 推理服务状态

使用方式：点击页面悬浮的"AI 助手"按钮，或在训练任务行点击"诊断"。

> **安全说明**：助手以**你的身份与权限**运行，只能访问你被分配的 namespace；所有写操作需人工确认；全部工具调用留有审计记录。未启用时入口自动隐藏，不影响控制台其余功能。
